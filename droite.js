//Cette fonction déplace le joueur a Droite
"use strict"
...
var deplace = function (req, res, query, fin) {
	...
	if ( tab.j.c > 0 ) {
		...
		if ( tab.m[tab.j.l][tab.j.c+1].type == "v" && tab.m[tab.j.l][tab.j.c+1].typeo == "v" ) {
		...
		} else if ( tab.m[tab.j.l][tab.j.c+1].type == "c" ) {
			for (x = 0 ; x < 20 ; x++ ) {
				for ( y = 0 ; y < 20 ; y++ ) {
					if ( tab.m[x][y].type == "c" ) {
			...
			tab.j.c++;
			tab.m[tab.j.l][tab.j.c+1].poid = 1;
		} else if ( tab.m[tab.j.l][tab.j.c+1].type == "ft" ) {
			if ( tab.m[tab.j.l][tab.j.c+1].etat == "f" ) {
